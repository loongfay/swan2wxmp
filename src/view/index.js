...
const parser = require('./plugins/parser').parser;
const stringify = require('./plugins/stringify');
const swan2wxml = require('./plugins/swan2wxml');
const utils = require('../lib/utils');
const getHtmlParser = require('./plugins/parser').getHtmlParser;
 ...
 */
function transformContent(path, contents, context) {
    return unified()
        .use(parser)
        .use(swan2wxml, {context})
        .use(stringify)
 ...
 */
module.exports.transform = function* transform(context) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        const result = yield transformContent(files[i], content, context);
