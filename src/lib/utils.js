...
function copyDirectory(fromPath, toPath) {
    const lists = fs.readdirSync(fromPath).filter(function (item) {
        return !/(node_modules|DS_store|idea)/i.test(item);
    });
    ...
    for (let i = 0; i < lists.length; i++) {
        ...
        arr.push(
            recursiveCopy(
                ...
                path.join(toPath, lists[i].replace(/swan$/, 'wxml').replace(/css$/, 'wxss')),
                options
            )
        );
    }
...
function renameFileExt(filePath) {
    if (/swan$/.test(filePath)) {
        return filePath.replace(/swan$/, 'wxml');
    }
    else if (/css$/.test(filePath)) {
        return filePath.replace(/css$/, 'wxss');
    }
    else {
        return filePath;
    }
